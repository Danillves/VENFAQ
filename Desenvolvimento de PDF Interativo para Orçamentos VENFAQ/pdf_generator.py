...
class VenfaqPDFGenerator:
    def __init__(self):
        self.styles = getSampleStyleSheet()
        self.setup_custom_styles()
        
    def setup_custom_styles(self):
        """Configurar estilos customizados para o PDF VENFAQ"""
    ...
    def gerar_pdf_orcamento(self, orcamento_data, itens_data, output_path):
        """Gerar PDF do orçamento baseado no template VENFAQ"""
    ...
    def _criar_cabecalho(self, orcamento_data):
        ...
        # Título VENFAQ
        elements.append(Paragraph("VENFAQ", self.styles['VenfaqTitle']))
        elements.append(Paragraph("ENGENHARIA E FACHADAS", self.styles['VenfaqSubtitle']))
        elements.append(Spacer(1, 20))
        
        # Título da proposta
        elements.append(Paragraph("P R O P O S T A", self.styles['VenfaqTitle']))
        elements.append(Spacer(1, 20))
    ...
    def _criar_info_cliente(self, orcamento_data):
        ...
        # Projeto
        elements.append(Paragraph(f"<b>PROJETO:</b> {orcamento_data.get('projeto_nome', 'N/A')}", self.styles['VenfaqSubtitle']))
        
        # Endereço
        endereco = orcamento_data.get('cliente_endereco', 'N/A')
        elements.append(Paragraph(f"<b>ENDEREÇO:</b> {endereco}", self.styles['VenfaqNormal']))
        elements.append(Spacer(1, 10))
        
        # Cliente
        elements.append(Paragraph(f"<b>CLIENTE:</b> {orcamento_data.get('cliente_nome', 'N/A')}", self.styles['VenfaqSubtitle']))
        elements.append(Spacer(1, 20))
    ...
    def _criar_item_detalhado(self, item, numero):
        ...
        elements.append(Paragraph(f"<b>{numero:02d}</b>", self.styles['VenfaqSubtitle']))
        elements.append(Spacer(1, 5))
        
        # Título do item
        elements.append(Paragraph(f"<b>{item.get('titulo', 'N/A')}</b>", self.styles['VenfaqSubtitle']))
        
        # Descrição técnica
        descricao = item.get('descritivo_tecnico', 'N/A')
        elements.append(Paragraph(descricao, self.styles['VenfaqNormal']))
        elements.append(Spacer(1, 10))
...
EXECUÇÃO DA OBRA - Será realizada com empresas instaladoras devidamente homologadas pela VENFAQ no prazo especificado de forma continua. Solicitações de interrupções de serviços, faseamento da execução ou qualquer outra solicitação do cliente de pausa ou atraso na execução serão orçadas à parte.
...
COMPATIBILIZAÇÃO DE PROJETOS - os valores podem ser reajustados em virtude da compatibilização do caderno técnico VENFAQ com os demais elementos construtivos.
...
ANDAIMES - Balancin elétrico (locação, montagem e desmontagem) - por conta do cliente.
        ...
        elements.append(Paragraph(condicoes, self.styles['VenfaqNormal']))
        elements.append(Spacer(1, 20))
    ...
    def _criar_observacoes_anexos(self, orcamento_data):
        ...
        if observacoes:
            elements.append(Paragraph("OBSERVAÇÕES", self.styles['VenfaqSubtitle']))
            elements.append(Paragraph(observacoes, self.styles['VenfaqNormal']))
            elements.append(Spacer(1, 15))
        
        # Anexos
        anexos = orcamento_data.get('anexos', '')
        if anexos:
            elements.append(Paragraph("ANEXOS", self.styles['VenfaqSubtitle']))
            elements.append(Paragraph(anexos, self.styles['VenfaqNormal']))
            elements.append(Spacer(1, 15))
        
        # Rodapé com informações da empresa
        elements.append(Spacer(1, 30))
        elements.append(Paragraph(
            "VENFAQ ENGENHARIA LTDA - CNPJ XX.XXX.XXX/0001-XX",
            self.styles['VenfaqNormal']
        ))
        elements.append(Paragraph(
